<# ============================================================
   deploy-project-links.ps1
   Adds Live Site + Repository buttons to About Me > Projects.

   Writes (with backups of any existing files):
     sources/UI/components/panels/AboutPanel.vue   (modified: + link buttons)
     sources/UI/components/panels/PanelShell.vue    (unchanged content, re-written for consistency)
     sources/UI/data/about.js                       (modified: + liveUrl/repoUrl fields)

   Uses no-BOM UTF8 writes (PowerShell 5.1-safe -- avoids the
   Set-Content BOM collision that breaks Rollup with "Unexpected
   character '»'").

   USAGE:
     1. Edit $ProjectRoot below if your repo lives elsewhere.
     2. Save this file as deploy-project-links.ps1
     3. Run:
          Unblock-File .\deploy-project-links.ps1
          powershell -ExecutionPolicy Bypass -File .\deploy-project-links.ps1
   ============================================================ #>
...
# ---- PanelShell.vue (unchanged from last deploy -- rewritten for consistency) ----
$panelShellContent = @'
<script setup>
...
# ---- AboutPanel.vue (modified: Live Site / Repository buttons added) ----
$aboutPanelContent = @'
<script setup>
...
.iw-about__project-link
{
    ...
    cursor: pointer;
    transition: color 0.2s var(--iw-ease), border-color 0.2s var(--iw-ease),
        background 0.2s var(--iw-ease), transform 0.2s var(--iw-ease);
}
...
.iw-about__project-link--live:hover,
.iw-about__project-link--live:focus-visible
{
    color: var(--iw-about-accent);
}

/* Disabled state -- shown when a project has no liveUrl/repoUrl yet.
   Same footprint as a real link so the row layout never jumps once the
   URL is filled in, but visually inert: dashed border, dimmed, no
...
# ---- about.js (modified: liveUrl / repoUrl fields added, empty placeholders) ----
$aboutJsContent = @'
...
Write-Host "  git diff sources/UI/data/about.js"
Write-Host "  git diff sources/UI/components/panels/AboutPanel.vue"
Write-Host "  npm run build"
Write-Host ""
Write-Host "Reminder: liveUrl / repoUrl in about.js are empty placeholders." -ForegroundColor Yellow
Write-Host "Fill them in per project to activate the Live Site / Repository buttons." -ForegroundColor Yellow
